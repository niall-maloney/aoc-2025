...
func main() {
	...
	current := matrix
	previous := createMatrix(len(matrix), len(matrix[0]))
	for sum(previous) != sum(current) {
		convolution := kernelConvolution(current)
		product(convolution, current)
